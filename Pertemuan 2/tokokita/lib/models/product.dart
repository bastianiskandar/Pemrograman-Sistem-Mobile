...
final List<Product> dummyProducts = [
  Product(id: 1, name: 'Laptop ASUS Vivo', price: 8500000, category: 'Elektronik', stock: 10, description: 'Core i5 RAM 8GB SSD 512GB'),
  Product(id: 2, name: 'Smartphone Galaxy', price: 4500000, category: 'Elektronik', stock: 4, description: 'Layar AMOLED 120Hz'),
  Product(id: 3, name: 'Kaos Polos Cotton', price: 75000, category: 'Fashion', stock: 25, description: 'Bahan Katun Combed 30s'),
  Product(id: 4, name: 'Jaket Hoodie Fleece', price: 220000, category: 'Fashion', stock: 3, description: 'Bahan Tebal dan Nyaman'),
  Product(id: 5, name: 'Nasi Goreng Spesial', price: 25000, category: 'Makanan', stock: 15, description: 'Porsi Komplit Telur & Ayam'),
  Product(id: 6, name: 'Kopi Susu Aren', price: 18000, category: 'Makanan', stock: 0, description: 'Espresso Susu Segar Aren Asli'),
  Product(id: 7, name: 'Headphone Wireless', price: 350000, category: 'Elektronik', stock: 8, description: 'Koneksi Bluetooth Bass Mantap'),
  Product(id: 8, name: 'Sepatu Sneakers Casual', price: 320000, category: 'Fashion', stock: 2, description: 'Desain Sporty Warna Putih'),
];
...
void tampilkanTabel(List<Product> list) {
  const garis = '+----+-------------------------+---------------+-------+----------------+---------------+';
  print(garis);
  print('| ID | ${'Nama Produk'.padRight(23)} | ${'Kategori'.padRight(13)} | ${'Stok'.padRight(5)} | ${'Harga'.padRight(14)} | ${'Status Stok'.padRight(13)} |');
  print(garis);
  for (var p in list) {
    print('| ${p.id.toString().padRight(2)} | ${p.name.padRight(23)} | ${p.category.padRight(13)} | ${p.stock.toString().padRight(5)} | ${p.formattedPrice.padRight(14)} | ${p.getStatusStok().padRight(13)} |');
  }
  print(garis);
...
void tambahProduk() {
  ...
  final String? desc = (descInput == null || descInput.isEmpty) ? null : descInput;

  final newId = products.isEmpty ? 1 : (products.map((p) => p.id).reduce((a, b) => a > b ? a : b) + 1);
  products.add(Product(id: newId, name: name, price: price, category: category, stock: stock, description: desc));
...
void beliProduk() {
  ...
  if (p.stock <= 0) {
    print('Pemberitahuan: Stok barang "${p.name}" sedang HABIS (${p.getStatusStok()})!\n');
    return;
  }
  ...
  if (qty > p.stock) {
    print('Error: Stok tidak mencukupi! Anda meminta $qty, namun stok hanya tersisa ${p.stock}.\n');
    return;
  }
  ...
  final hargaSetelahDiskonSatuan = hitungHargaSetelahDiskon(p.price, persenDiskon: persenDiskon);
...
void ujiPraktikum() {
  ...
  print('- int, double, String, bool: $contohNama | Stok: $contohStok | Harga: ${formatRupiah(contohHarga)} | Tersedia: $statusTersedia');
  ...
  print('- Aritmatika (+, -, *, /, %):');
  print('  * Total $beliItem item: ${formatRupiah(totalAritmatika)} (Operator *)');
  ...
  print('  * Stok laptop == Stok smartphone? ${dummyProducts[0].stock == dummyProducts[1].stock}');
  print('- Logika (&&, ||, !):');
  ...
  print('  * Stok ${dummyProducts[0].stock} : ${dummyProducts[0].getStatusStok()}');
  print('  * Stok ${dummyProducts[1].stock} : ${dummyProducts[1].getStatusStok()}');
  print('  * Stok ${dummyProducts[5].stock} : ${dummyProducts[5].getStatusStok()}');
  print('- switch-case (Diskon Kategori):');
  for (var k in kategoriList) {
    print('  * Kategori $k -> diskon: ${hitungDiskonKategori(k)}%');
  }
  print('- for loop (Total Belanja Dummy): ${formatRupiah(hitungTotalBelanja(dummyProducts))}');
  ...
  print('  * Harga Rp 250.000 tanpa argumen diskon : ${formatRupiah(hTanpaDiskon)}');
  ...
  print('- Instance Product: ID ${pContoh.id} | ${pContoh.name} | Harga: ${pContoh.formattedPrice} | Status: ${pContoh.getStatusStok()}');
  print('- Null Safety: pContoh.description bernilai null -> "${pContoh.description}" (Diizinkan karena bertipe String?)');
  ...
  print('  * Diskon: ${dp.discountPercent}% | Harga Final: ${dp.formattedFinalPrice}');

  print('\n[TUGAS MANDIRI]');
  print('- 1. Method tambahan getStatusStok(): Sukses diimplementasikan pada class Product.');
  print('- 2. List<Product> 8 data dummy: ${dummyProducts.length} produk siap digunakan.');
  print('- 3. Total belanja 8 produk dummy: ${formatRupiah(hitungTotalBelanja(dummyProducts))}');
  print('============================================================\n');
}
